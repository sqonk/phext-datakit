...
class CSVImporterTest extends TestCase
{
    ...
    public function testSkipRows(): void 
    {
        ...
        $importer = new CSVImporter(input:$data, inputIsRawData:true, headersAreFirstRow:true, skipRows:5);
        $this->expectError(E_USER_WARNING);
        $this->expectErrorMessage('Attempt to skip past beyond the last row of the CSV.');
        $this->assertSame(true, $importer->validate());
        
        $importer = new CSVImporter(input:__DIR__.'/iris-h.csv', headersAreFirstRow:true, skipRows:5);
        $this->expectError(E_USER_WARNING);
        $this->expectErrorMessage('Attempt to skip past beyond the last row of the CSV.');
        $this->assertSame(true, $importer->validate());
    }
