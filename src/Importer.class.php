...
class Importer
{
   ...
   */
  public static function csv_data(?callable $callback, string $data, bool $headersAreFirstRow = false, ?array $customHeaders = null): bool|array
  {
   ...
   */
  public static function csv_file(?callable $callback, string $filePath, bool $headersAreFirstRow = false, ?array $customHeaders = null, int $skipRows = 0): bool|array
  {
    $importer = new CSVImporter(
      input:$filePath,
      headersAreFirstRow:$headersAreFirstRow,
   ...
   * This method will throw a `RuntimeException` if the file can not be opened for any reason.
   */
  public static function yield_csv(string $filePath, bool $headersAreFirstRow = false, ?array $customHeaders = null, int $skipRows = 0): \Generator
  {
    $importer = new CSVImporter(
      input:$filePath,
      headersAreFirstRow:$headersAreFirstRow,
   ...
   */
  public static function csv_dataframe(string $filePath, array|bool $headers = false, int $skipRows = 0): ?DataFrame
  {
    if (is_array($headers)) {
   ...
   * at least 1 line of applicable data.
   */
  public static function delimitered_data(?callable $callback, string $data, string $itemDelimiter, string $lineDelimiter = "\n", bool $headersAreFirstRow = false, ?array $customHeaders = null): bool|array
  {
   ...
   */
  public static function mysql_dataframe(string $database, string $source, string $server = 'localhost', string $username = 'root', string $password = ''): ?DataFrame
  {
   ...
   * @return ?DataFrame A DataFrame containing the resulting rows. Returns NULL if the specified table or query returns no rows.
   */
  public static function sqlite_dataframe(string $filepath, string $source): ?DataFrame
  {
    $source = trim($source);
    $lower = strtolower($source);
